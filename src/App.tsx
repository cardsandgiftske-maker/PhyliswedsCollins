...
import { Calendar, MapPin, Gift, Shirt, Sparkles, Mail, Camera, ChevronUp } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero-section', 'maps-section', 'program-section', 'dress-code-section', 'gifting-section', 'gallery-section', 'rsvp-section'];
    ...
    <>
      ...
      <header className="fixed top-0 inset-x-0 z-40 bg-[#FCFAF7]/90 backdrop-blur-md border-b border-stone-200/50 transition-all">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            <span className="text-maroon-800">PHYLIS</span>
            <span className="text-stone-400 font-sans text-xs italic">&amp;</span>
            <span className="text-maroon-800">COLLINS</span>
          </button>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-full transition-all flex items-center gap-1.5 cursor-pointer hover:text-stone-950 ${
          ...
          <button
            ...
            className="md:hidden px-4 py-1.5 bg-maroon-800 hover:bg-maroon-900 text-white font-sans font-extrabold text-[10px] uppercase tracking-widest rounded-full transition-all cursor-pointer shadow-sm"
          >
            RSVP NOW
          </button>
        </div>
      ...
      <div className="hidden lg:flex fixed right-8 top-1/2 -translate-y-1/2 z-40 flex-col gap-4 items-center">
        {navItems.map((item) => (
          <button
            ...
            title={item.label}
          >
            <span className="absolute right-full mr-4 bg-white/95 border border-stone-200 px-2.5 py-1 rounded text-[10px] font-sans font-bold uppercase tracking-wider shadow-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 text-maroon-800">
              {item.label}
            </span>
            <span className={`w-2.5 h-2.5 rounded-full border transition-all ${
              activeSection === item.id 
                ? 'bg-maroon-800 border-maroon-600 scale-125' 
      ...
      <AnimatePresence>
        {showScrollTop && (
          ...
          >
            <button
              onClick={() => scrollToSection('hero-section')}
              className="p-3 bg-white hover:bg-stone-50 border border-stone-200/80 text-maroon-800 rounded-full shadow-lg active:scale-95 transition-all cursor-pointer"
              title="Scroll to Top"
            >
